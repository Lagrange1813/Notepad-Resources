...
class PureTextView: UIScrollView, UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        let titleToStore: String = titleView.text
        let bodyToStore: String = bodyView.text
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        if text == "\n" {
            if isShortcutBtnInputing && titleViewUnderEditing {
                let location = titleView.selectedRange.location
                titleView.selectedRange = NSRange(location: location + 1, length: 0)
