...
class SideMenuVC: UIViewController {
    ...
    func configureDataSource() {
        ...
        let textCellRegistration = UICollectionView.CellRegistration<UICollectionViewListCell, TextItem>{
            cell, _, textItem in
        ...
        let blankCellRegistration = UICollectionView.CellRegistration<UICollectionViewListCell, BlankItem>{
            cell, _, _ in
        ...
        dataSource = UICollectionViewDiffableDataSource<Section, ListItem>(collectionView: textList) {
            ...
            case .section(let sectionItem):
                ...
                let cell = collectionView.dequeueConfiguredReusableCell(using: sectionCellRegistration, for: indexPath, item: sectionItem)
                return cell

            case .book(let bookItem):

                // Dequeue header cell
                let cell = collectionView.dequeueConfiguredReusableCell(using: bookCellRegistration, for: indexPath, item: bookItem)
            ...
            case .text(let textItem):
                ...
                // Dequeue symbol cell
                let cell = collectionView.dequeueConfiguredReusableCell(using: textCellRegistration, for: indexPath, item: textItem)
                var background = UIBackgroundConfiguration.listPlainCell()
                background.backgroundColor = .systemBackground
                cell.backgroundConfiguration = background
            ...
            case .blank(let blankItem):
                ...
                let cell = collectionView.dequeueConfiguredReusableCell(using: blankCellRegistration, for: indexPath, item: blankItem)
                return cell
            }
        }
    }
    ...
    func setupSnapshots() {
        ...
        for bookItem in mainItems {
            ...
            let textListItemArray = bookItem.texts.map {
                ListItem.text($0)
            }
